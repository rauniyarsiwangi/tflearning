...
data "azurerm_storage_container" "storcont" {
  count                = var.enable_backup == true ? 1 : 0
...
data "azurerm_storage_account_blob_container_sas" "main" {
  ...
  container_name    = data.azurerm_storage_container.storcont.0.name
  https_only        = true
...
resource "azurerm_app_service_plan" "main" {
  count               = var.enable_plan == true ? 1 : 0
...
data "azurerm_app_service_plan" "existing_plan" {
  count               = var.enable_plan == false ? 1 : 0
  name                = var.app_service_plan_name
...
resource "azurerm_app_service" "main" {
  ...
  resource_group_name     = local.resource_group_name
  location                = local.location
  app_service_plan_id     = var.enable_plan == true ? azurerm_app_service_plan.main.0.id : data.azurerm_app_service_plan.existing_plan.0.id
  ...
  dynamic "backup" {
    ...
    content {
      ...
      storage_account_url = format("https://${data.azurerm_storage_account.storeacc.0.name}.blob.core.windows.net/${data.azurerm_storage_container.storcont.0.name}%s", data.azurerm_storage_account_blob_container_sas.main.0.sas)
      schedule {
